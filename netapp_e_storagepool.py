...
class NetAppESeriesStoragePool(object):
    ...
    def filter_drives(
        ...
        if spindle_speed != None:  # 0 is valid for ssds
    ...
    def _candidate_set_passes(self, candidate_set, min_capacity_bytes=None, min_drive_count=None,
        ...
        if min_capacity_bytes != None and self._calculate_usable_capacity(int(candidate_set[0]['rawCapacity']),
                                                                          len(candidate_set),
    ...
    def get_storage_pool(self, storage_pool_name):
        ...
        try:
            ...
            if rc == 404 and self.state == 'absent':
                ...
                err = get_exception()
                self.module.exit_json(
                    msg="Failed to get storage pools. Array id [%s].  Error[%s]. State[%s]. RC[{3}]." % (
    ...
    def get_candidate_disks(self):
        ...
        try:
            ...
            self.module.exit_json(
                msg="Failed to fetch disk drives. Array id [%s].  Error[%s]." % (pool_id, self.ssid, str(err)))
