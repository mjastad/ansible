...
options:
  ...
  api_username:
    required: true
    description:
    - the user name for authentication into the web services proxy
  api_password:
    required: true
    description:
    - the password for authentication into the web services proxy
  api_url:
    required: true
    description:
    - the resolvable endpoint for the web services proxy. Example, http://10.10.10.10:8080/devmgr/v2
'''
EXAMPLES = '''
    - name: Configure Hostgroup
      na_eseries_hostgroup:
        ssid: "{{ ssid }}"
    ...
    if state == 'present':
        if not exists:
            try:
                ...
                err = get_exception()
                module.fail_json(msg="Failed to create a host group. Id [%s]. " +
                                     "Error [%s]." % (name, ssid, str(err)))
            ...
            if set(current_hosts) != set(hosts):
                try:
                    ...
                    err = get_exception()
                    module.fail_json(msg="Failed to update host group. Group: [%s]. Id [%s]. " +
                                         "Error [%s]." % (name, ssid, str(err)))
                module.exit_json(changed=True, hosts=hosts, **data)
            else:
        ...
        if exists:
            ...
            try:
                ...
                err = get_exception()
                module.fail_json(msg="Failed to delete host group. Group: [%s]. Id [%s]. " +
                                     "Error [%s]." % (name, ssid, str(err)))
